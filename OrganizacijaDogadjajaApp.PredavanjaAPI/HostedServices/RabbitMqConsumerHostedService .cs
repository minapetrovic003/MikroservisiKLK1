    ...
    public sealed class RabbitMqConsumerHostedService : BackgroundService
    {
        ...
        private async Task HandleMessageAsync(BasicDeliverEventArgs ea, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("Primljena nevalidna poruka, ignorisemo je.");
                ...
                if (!alreadyProcessed)
                {
                    ...
                    if (predavac != null)
                    {
                        ...
                        {
                            Id = Guid.NewGuid(),
                            Tema = $"Predavanje za: {eventData.NazivDogadjaja}",
                            TrajanjePredavanja = eventData.Trajanje,
                            Pocetak = eventData.DatumIVreme,
                    ...
                    _logger.LogInformation("Predavanje kreirano za dogadjaj {DogadjajId}", eventData.DogadjajId);
                }
                else
                {
